    ...
    public class UsbEventWatcher : IUsbEventWatcher
    {
        ...
        private static UsbDevice GetUsbDevice(string USBControllerDeviceID, string PnPEntityDeviceID)
        {
...
#if DEBUG
            ManagementObjectSearcher Win32_PnPEntity = new ManagementObjectSearcher($"SELECT * FROM Win32_PnPEntity WHERE DeviceID LIKE '%{serial}%'");
#else
            ManagementObjectSearcher Win32_PnPEntity = new ManagementObjectSearcher($"SELECT Caption, ClassGuid, Description, DeviceID, Manufacturer FROM Win32_PnPEntity WHERE DeviceID LIKE '%{serial}%'");
#endif
            ...
            foreach (ManagementObject entity in Win32_PnPEntity.Get())
            {
                ...
                ManagementObjectSearcher Win32_DiskDrive = new ManagementObjectSearcher(
            ...
            ManagementObjectSearcher Win32_USBHub = new ManagementObjectSearcher($"SELECT * FROM Win32_USBHub WHERE DeviceID LIKE '%{serial}%'");
            ...
            if (USBHubCollection.Count > 0)
            {
                ...
                while (++attempts < 1000)
                {
                    ...
                    if (string.IsNullOrEmpty(usbDevice.MountedDirectoryPath))
                    {
                        ...
                        ManagementObjectSearcher associators = new ManagementObjectSearcher(
        ...
        private static string GetDiskDrivePath(string diskDriveDeviceID)
        {
            ...
            ManagementObjectSearcher Win32_DiskDriveToDiskPartition = new ManagementObjectSearcher(
            ...
            {
                ManagementObjectSearcher Win32_LogicalDiskToPartition = new ManagementObjectSearcher(
